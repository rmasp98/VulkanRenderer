...
int main() {
  ...
  auto pipelineId = renderer.RegisterPipeline(
      {{{vk::ShaderStageFlagBits::eVertex, LoadShader("../shaders/vert.spv")},
  ...
  std::unique_ptr<IndexBuffer<ColouredVertex2D>> buffer2(
      new IndexBuffer<ColouredVertex2D>({{{-0.5f, -0.5f}, {1.0f, 0.0f, 0.0f}},
                                         {{0.5f, -0.5f}, {0.0f, 1.0f, 0.0f}},
                                         {{0.5f, 0.5f}, {0.0f, 0.0f, 1.0f}},
